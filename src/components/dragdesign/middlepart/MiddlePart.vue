<template>
    <el-main id="formDesignMiddle">
          <el-button type="text" @click="preview=true">预览</el-button>
          <el-button type="text">发布</el-button>
          <el-button type="text">导入json</el-button>
          <el-button type="text" @click="dialogShow=true">导出json</el-button>
          <el-button type="text" @click="resultForm=[]">清空</el-button>
          <el-form :model="bindvalues"
              ref="bindValue"
              class="demo-dynamic">
            <draggable
              v-model="resultForm"
              ...
              :sort="true">
                <transition-group style="height:660px; display: block;">
                  <template v-for="(item, itemIndex) in resultForm">
                        ...
                        class="designPart">
                          <transition-group style="min-height:60px; display: block;">
                            <template v-for="(span, index) in col.list">
                              <div :key="span.key" @click.stop="handleItemInfo(span)" class="middleDragItem">
                                <i class="el-icon-close" @click.self.stop="handleDeleteForm(col.list, index)"></i>
                                <process-form :preInfo="span" :bindValues="bindvalues"></process-form>
                              </div>
                            </template>
                          </transition-group>
                        </draggable>
                    ...
                    <div :key="item.key" v-else @click="handleItemInfo(item)" class="middleDragItem" >
                     <i class="el-icon-close" @click.self.stop="handleDeleteForm(resultForm, itemIndex)"></i>
                      <process-form :key="item.key" :preInfo="item" :bindValues="bindvalues"></process-form>
                    </div>
                  </template>
                </transition-group>
            </draggable>
          ...
          <el-dialog
            ...
            <json-show v-if="dialogShow" :data="resultForm" :readOnly="true"></json-show>
          ...
          <el-dialog
            ...
            v-dialogDrag="{delay:300}"
            :close-on-click-modal="false">
            <dynamic-form-show :activeForm="resultForm"></dynamic-form-show>
            <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    async handleCopy() {
      let clipboardUrl = new clipboard(".copyClass");
