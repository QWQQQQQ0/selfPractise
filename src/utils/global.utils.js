const globalUtils = {
    ...
    Object.keys(data).forEach(function(key) {
      if (key.startsWith('checkbox') || key.startsWith('multipleSelect')) {
        if (data[key] !== null && data[key] !== '') {
          const tempArray = data[key].split(',');
  ...
  formatDynCols(data, imageUrl) {
    Object.keys(data).forEach(function(key) {
      if (key.startsWith('checkbox') || key.startsWith('multipleSelect')) {
        ...
        if (data[key] !== null && data[key] !== '' && data[key] !== undefined && data[key] instanceof Array) {
          let str = '';
          for (let i = 0, n = data[key].length; i < n; i++) {
  ...
  isDomain(val) {
    ...
    const regNeg = /((https|http|ftp|rtsp|mms):\/\/)?(([0-9a-zA-Z_!~*'().&=+$%-]+:)?[0-9a-zA-Z_!~*'().&=+$%-]+@)?(([0-9]{1,3}\\.){3}[0-9]{1,3}|([0-9a-zA-Z_!~*'()-]+\\.)*([0-9a-zA-Z][0-9a-zA-Z-]{0,61})?[0-9a-zA-Z]\\.[a-zA-Z]{2,6})(:[0-9]{1,4})?((\/?)|(\/[0-9a-zA-Z_!~*'().;?:@&=+$,%#-]+)+\/?)/g
      ;
  ...
  isNull(T) {
    var res = false;
    if (this.isUndefined(T) || T === '' || T === null) {
      res = true;
    }
    return res;
  },
  ...
  isInvalidValue(T) {
    var res = false;
    if (T === undefined || T === 'null' || T === null || T === 'undefined') {
      res = true;
    }
    return res;
  },
  ...
  isSameDate(date1, date2, fmt) {
    const d1 = this.formatDate(date1, fmt);
    const d2 = this.formatDate(date2, fmt);
    return d1 === d2;
  },
  // 判断校验8到16位数字与密码组合的密码
  isValidPwd(T) {
    // let reg = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,16}$/;
    const reg = /^(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[/`~!@#$%^&*()\-+_=\\|[\]{};:'",<.>/? ]).{6,}$/;
    return reg.test(T);
  },
  // 判断是否为电话号码
  isTel(T) {
